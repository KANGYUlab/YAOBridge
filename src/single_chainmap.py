def single_ref_to_query_pos(tree,chrom,pos):
    yao_pos = []
    #yao_pos.append([aln['qname'],qpos,aln['strand'],aln['Confidence']])
...
def single_query_to_ref_pos(tree,chrom,pos):
    hg38_pos=[]
    # overlap 返回的是一个 set，包含所有覆盖该位点的 Interval 对象
    hits = tree.at(pos) # 或者用 .overlap(pos, pos+1)
    if not hits:
